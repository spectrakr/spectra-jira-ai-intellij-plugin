...
//            {cf[10020]: 838}
    ...
    private void setupEpicLabelHandlers() {
        epicLabel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (currentEditingIssue != null && epicLabel.isEnabled()) {
            ...
            public void mouseExited(MouseEvent e) {
                if (currentEditingIssue != null && epicLabel.isEnabled()) {
                    ...
                    if (currentEditingIssue.getEpicColor() != null) {
                        try {
                            Color epicColor = Color.decode(currentEditingIssue.getEpicColor());
                            epicLabel.setBackground(epicColor);
                        } catch (NumberFormatException ex) {
                            epicLabel.setBackground(UIManager.getColor("Label.background"));
                        }
                    } else if (currentEditingIssue.getParentKey() != null) {
    ...
    private void updateParent(String parentKey) {
        ...
        String oldParentKey = currentEditingIssue.getParentKey();
        String oldParentSummary = currentEditingIssue.getParentSummary();
        String oldEpicColor = currentEditingIssue.getEpicColor();
        ...
        JiraService jiraService = getConfiguredJiraService();
        jiraService.updateIssueParentAsync(currentEditingIssue.getKey(), parentKey)
            .thenRun(() -> SwingUtilities.invokeLater(() -> {
                // Update the current issue model
                currentEditingIssue.setParentKey(parentKey);
                ...
                if (parentKey != null && !parentKey.isEmpty()) {
                    ...
                    jiraService.getIssueAsync(parentKey)
                        .thenAccept(parentIssue -> SwingUtilities.invokeLater(() -> {
                            currentEditingIssue.setParentSummary(parentIssue.getSummary());
                            currentEditingIssue.setEpicColor(parentIssue.getEpicColor());
                            updateEpicLabelDisplay();
                            updateStatus("Parent updated successfully for " + currentEditingIssue.getKey());
                        }))
                        .exceptionally(ex -> {
                            SwingUtilities.invokeLater(() -> {
                                // Even if we can't get epic details, still update the key
                                currentEditingIssue.setParentSummary(null);
                                currentEditingIssue.setEpicColor(null);
                                updateEpicLabelDisplay();
                                updateStatus("Parent updated (details unavailable) for " + currentEditingIssue.getKey());
                            });
                            return null;
                        });
                } else {
                    ...
                    currentEditingIssue.setEpicColor(null);
                    updateEpicLabelDisplay();
                    updateStatus("Parent removed successfully for " + currentEditingIssue.getKey());
                }
            }))
            .exceptionally(throwable -> {
                SwingUtilities.invokeLater(() -> {
                    // Restore old values on error
                    currentEditingIssue.setParentKey(oldParentKey);
                    currentEditingIssue.setParentSummary(oldParentSummary);
                    currentEditingIssue.setEpicColor(oldEpicColor);
                    updateEpicLabelDisplay();
    ...
    private void updateEpicLabelDisplay() {
        ...
        if (currentEditingIssue.getParentKey() != null && currentEditingIssue.getParentSummary() != null) {
            String epicText = currentEditingIssue.getParentKey() + " " + currentEditingIssue.getParentSummary();
            epicLabel.setText(epicText);
