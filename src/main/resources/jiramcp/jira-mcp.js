...
var require_uri_all = __commonJS({
  "node_modules/uri-js/dist/es5/uri.all.js"(exports, module) {
    (function(global2, factory2) {
      typeof exports === "object" && typeof module !== "undefined" ? factory2(exports) : typeof define === "function" && define.amd ? define(["exports"], factory2) : factory2(global2.URI = global2.URI || {});
...
var JiraApiClient = class {
   ...
   * JiraApiClient 초기화
   *
   * @param apiToken - Jira API 토큰 (없으면 설정 파일이나 환경변수에서 가져옴)
   * @param email - Jira 계정 이메일 (없으면 설정 파일이나 환경변수에서 가져옴)
   * @param baseUrl
   */
  constructor(apiToken, email, baseUrl) {
    this.apiToken = apiToken || "";
    this.email = email || "";
    this.baseUrl = baseUrl || "";
    if (!this.apiToken || !this.email || !this.baseUrl) {
      throw new Error(
        "Jira API \uC124\uC815\uC774 \uC644\uB8CC\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4. \uC778\uC99D \uC815\uBCF4\uB97C \uD655\uC778\uD558\uC138\uC694."
      );
    }
  ...
  async makeRequest(method, endpoint, params, data) {
    ...
    try {
      ...
      if (axios_default.isAxiosError(error)) {
        const errorMessage = `Jira API \uC624\uB958: ${(_a = error.response) == null ? void 0 : _a.status} - ${((_b = error.response) == null ? void 0 : _b.data) || error.message}`;
...
var jiraClient;
try {
  const { token, email, baseUrl } = parseArgs();
  jiraClient = new JiraApiClient(token, email, baseUrl);
} catch (error) {
  console.error("Jira \uD074\uB77C\uC774\uC5B8\uD2B8 \uCD08\uAE30\uD654 \uC624\uB958:", error);
  process.exit(1);
}
...
server.tool(
  ...
  "\uD2B9\uC815 \uC774\uC288\uC758 \uC0C1\uC138 \uC815\uBCF4\uB97C \uC870\uD68C\uD569\uB2C8\uB2E4.",
  {
    issue_key: external_exports.string().describe("\uC774\uC288 \uD0A4 (\uC608: PROJ-123)")
  },
  async ({ issue_key }) => {
    try {
      const issue = await jiraClient.getIssueDetails(issue_key);
      const fields = issue.fields;
      let result = `Key: ${issue_key}
`;
...
`;
      ...
      const assigneeName = assignee ? assignee.displayName : "\uBBF8\uD560\uB2F9";
      const assigneeEmail = assignee ? assignee.emailAddress : "";
      result += `Assignee: ${assigneeName}${assigneeEmail ? ` (${assigneeEmail})` : ""}
`;
      let descriptionText = "\uC124\uBA85 \uC5C6\uC74C";
      if (fields.description) {
        if (typeof fields.description === "object" && fields.description.content) {
          descriptionText = JSON.stringify(fields.description.content);
        } else {
...
`;
      ...
      const priorityName = priority ? priority.name : "\uBBF8\uC124\uC815";
...
`;
      ...
      console.error(`\uB3C4\uAD6C \uC2E4\uD589 \uC911 \uC624\uB958 \uBC1C\uC0DD:`, error);
      return {
        content: [
          {
            type: "text",
